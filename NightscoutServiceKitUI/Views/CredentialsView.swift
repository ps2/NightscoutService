...
struct CredentialsView: View, HorizontalSizeClassOverride {
    ...
    var body: some View {
        VStack {
            Text(LocalizedString("Nightscout Login",comment: "Nightscout login view"))
                .font(.largeTitle)
                .fontWeight(.semibold)
            ...
            Button(action: { self.viewModel.attemptAuth(urlString: self.url, apiSecret: self.apiSecret) } ) {
                if self.viewModel.isVerifying {
                    ...
                    Text(LocalizedString("Login",comment: "Login button"))
                }
            }
            ...
            if allowCancel {
                Button(action: { self.viewModel.didCancel?() } ) {
                    Text(LocalizedString("Cancel",comment: "Cancel")).padding(.top, 20)
                }
            }
        }
